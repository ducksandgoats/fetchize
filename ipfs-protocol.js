export default async function makeIPFSFetch (opts = {}) {
    ...
    const app = await (async () => { if (finalOpts.helia) { return finalOpts.helia; } else {const {createHelia} = await import('helia');const {FsDatastore} = await import('datastore-fs');const {FsBlockstore} = await import('blockstore-fs');return await createHelia({blockstore: new FsBlockstore(repo), datastore: new FsDatastore(repo), libp2p: {transports: [circuitRelayTransport({discoverRelays: 0,reservationConcurrency: 1})], services: {dht: kadDHT(), pubsub: gossipsub(), identify: identify(), relay: circuitRelayServer({hopTimeout: 30 * 1000,advertise: true,reservations: {maxReservations: 15,reservationClearInterval: 300 * 1000,applyDefaultLimit: true,defaultDurationLimit: 2 * 60 * 1000,defaultDataLimit: BigInt(2 << 7),maxInboundHopStreams: 32,maxOutboundHopStreams: 64}})}}});} })()
